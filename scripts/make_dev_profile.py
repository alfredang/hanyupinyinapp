...
PROFILE_NAME = "HanyuPinyin Dev " + BUNDLE.replace(".", "-")
...
dev_certs = [c for c in certs if c["attributes"]["certificateType"] in ("DEVELOPMENT", "IOS_DEVELOPMENT")]
if not dev_certs:
...
# 2. Device (register if missing)
devices = find("/v1/devices?limit=200")
dev = next((d for d in devices if d["attributes"]["udid"].lower() == UDID.lower()), None)
if dev:
    device_id = dev["id"]
    print(f"device: existing {device_id}")
else:
    s, d = call("POST", "/v1/devices", {"data": {"type": "devices", "attributes": {
        "name": DEVNAME, "platform": "IOS", "udid": UDID}}})
    if s not in (200, 201):
        sys.exit(f"register device -> {s}: {d}")
    device_id = d["data"]["id"]
